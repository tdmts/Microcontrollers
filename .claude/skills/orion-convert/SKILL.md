...
for the full component reference, and [`Labo1/Morsecode.html`](../../../Labo1/Morsecode.html)
for a real, correctly-converted example).
...
6. **If the target file already exists**, ask for a plain yes/no confirmation
   before overwriting it. No diff needed — just confirm intent.
7. **After writing**, summarize in plain text: which components you used,
   any placeholder values that still need to be filled in (see Evaluation
   block below), and any source `src=` paths/URLs the user should double
   check.
...
- **Checklist — always, on every exercise page:** any page that describes a
  ...
  `LaboN/` page this skill produces) gets a `.info-box.evaluation` checklist
  block as the **last thing in `.container`**, regardless of whether the
  source text mentions grading/submission/indienen at all. Don't gate this
  on keywords — it's part of the standard page structure now, same as the
  `<h1>`/lead paragraph. Skip it only for pages that clearly aren't a
  student exercise (e.g. `template.html` itself, a pure reference/index
  page). It's framed as **self-evaluation**, not a formal evaluation being
  executed on/by the page:
